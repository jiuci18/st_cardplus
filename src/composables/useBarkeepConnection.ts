...
export function useBarkeepConnection() {
  ...
  const persistSession = () => {
    if (!session.value || session.value.mode !== "standalone") {
      localStorageStore.remove(BARKEEP_SESSION_STORAGE_KEY);
      return;
    }
  ...
  const ping = async (): Promise<boolean> => {
    ...
    try {
      ...
      ElMessage.success("Barkeep Ping 成功");
      return true;
    } catch (error) {
      status.value = null;
      errorMessage.value =
  ...
  const login = async (): Promise<boolean> => {
    ...
    try {
      ...
      persistConfig();
      persistSession();
      ElMessage.success("Barkeep 登录成功，正在 Ping");
    } catch (error) {
      session.value = null;
      localStorageStore.remove(BARKEEP_SESSION_STORAGE_KEY);
      errorMessage.value =
  ...
  const initialize = () => {
    ...
    if (
      ...
      savedSession.session?.mode === "standalone" &&
