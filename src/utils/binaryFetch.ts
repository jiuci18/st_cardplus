...
const bytesToBlob = (bytes: Uint8Array, mimeType: string): Blob => {
  ...
  copied.set(bytes);
  const arrayBuffer = copied.buffer.slice(copied.byteOffset, copied.byteOffset + copied.byteLength);
...
const normalizeMimeType = (mimeType: string | null | undefined, fileName: string): string => {
...
const assertExpectedImage = (mimeType: string, url: string) => {
  if (mimeType && mimeType !== 'application/octet-stream' && !mimeType.toLowerCase().startsWith('image/')) {
    throw new Error(`链接返回的内容不是图片: ${url}`);
  }
};

const fetchBinaryViaBrowser = async (url: string, options: BinaryFetchOptions): Promise<BinaryFetchResult> => {
...
const fetchBinaryViaTauri = async (url: string, options: BinaryFetchOptions): Promise<BinaryFetchResult> => {
  const { invoke } = await import('@tauri-apps/api/core');
  const result = await invoke<{ base64_data: string; file_name: string; mime_type: string }>('fetch_binary', {
    url,
...
export const fetchBinaryResource = async (
  ...
  if (options.preferDesktopBackend !== false && isTauriApp() && isHttpUrl(url)) {
...
export const binaryFetchResultToBlob = (result: BinaryFetchResult): Blob => bytesToBlob(result.bytes, result.mimeType);
