<template>
  <SidebarTreePanel title="角色列表" :tree-data="treeData" :tree-props="treeProps"
    ...
    :allow-drag="allowDrag" :allow-drop="allowDrop" :handle-node-drop="handleNodeDrop" @node-click="handleNodeClick">
    ...
    <template #node="{ node, data }">
      <div class="sidebar-tree-node">
        ...
        <div v-if="data.nodeType === 'character'" class="sidebar-tree-node-actions">
          <el-tooltip content="删除角色" placement="top">
            <button @click.stop="emit('delete', data.id)" class="sidebar-tree-node-action-button is-danger">
              <Icon icon="ph:trash-duotone" />
            </button>
          </el-tooltip>
        </div>
        <div v-if="data.nodeType === 'project'" class="sidebar-tree-node-actions">
          <el-tooltip content="重命名项目" placement="top">
            <button @click.stop="emit('rename-project', data.raw.id)" class="sidebar-tree-node-action-button">
