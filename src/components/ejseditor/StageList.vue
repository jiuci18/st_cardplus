<template>
  <div class="stage-list-container">
    <div class="list-header">
      ...
      <el-input :model-value="props.logicBlock.defaultStageContent" @update:model-value="updateDefaultContent"
        size="small" placeholder="默认情况下的内容" class="default-content-input">
    ...
    <div v-if="localStages.length === 0" class="empty-state">
      <el-empty description="暂无阶段" :image-size="50">
        <el-button type="primary" @click="addStage" size="small">
          添加第一个阶段
        </el-button>
      </el-empty>
    </div>

    <draggable v-else :list="localStages" item-key="id" handle=".drag-handle" animation="200" ghost-class="ghost"
      chosen-class="chosen" class="stage-list" @end="onDragEnd">
      <template #item="{ element: stage, index }">
        <div class="stage-item-wrapper">
          <div class="stage-item" :class="{ 'is-expanded': expandedStageId.includes(stage.id) }"
            ...
            <div class="stage-header">
              <div class="stage-info">
                ...
                <el-tooltip :content="stage.name" placement="top" :enterable="false">
                  <el-icon class="info-icon" @click.stop>
                    <InfoFilled />
                  </el-icon>
                </el-tooltip>
              </div>
...
const previousStageIds = ref<string[]>([]);
...
watch(
  ...
  (newStages) => {
    ...
    const nextStageIds = newStages.map((stage) => stage.id);
    const nextStageIdSet = new Set(nextStageIds);
    const preserved = expandedStageId.value.filter((id) => nextStageIdSet.has(id));
    const previousStageIdSet = new Set(previousStageIds.value);
    const addedStageIds = nextStageIds.filter((id) => !previousStageIdSet.has(id));

    addedStageIds.forEach((id) => {
      if (!preserved.includes(id)) {
        preserved.push(id);
      }
    });

    expandedStageId.value = preserved;
    previousStageIds.value = nextStageIds;
  },
