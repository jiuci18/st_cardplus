<template>
  <div class="basic-info-panel">
    <el-form :model="character.data" label-position="top" class="basic-info-form">
      <el-form-item label="角色名">
        <el-input :model-value="character.data.name" @update:model-value="updateField('name', $event)"
          placeholder="请输入角色名" />
      </el-form-item>

      <el-form-item label="标签">
        <el-select ref="tagsSelectRef" :model-value="character.data.tags"
          @update:model-value="updateField('tags', $event)" multiple filterable allow-create default-first-option
      ...
      <el-form-item>
        ...
        <el-input :model-value="character.data.description" @update:model-value="updateField('description', $event)"
          type="textarea" :rows="10" placeholder="角色的详细描述" />
      </el-form-item>

      <el-form-item label="开场白">
        <el-input :model-value="character.data.first_mes" @update:model-value="updateField('first_mes', $event)"
          type="textarea" :rows="8" placeholder="角色的第一句话" />
      </el-form-item>
    </el-form>

    <el-dialog v-model="dialogVisible" title="从角色信息选择" width="680px" :close-on-click-modal="false">
      <el-empty v-if="characterInfoOptions.length === 0" description="暂无可用的角色信息" />

      <div v-else class="selector-layout">
        <el-scrollbar max-height="420px">
          <div class="selector-list">
            <div v-for="option in characterInfoOptions" :key="option.id" class="selector-item"
              :class="{ 'is-selected': selectedCharacterId === option.id }" @click="selectedCharacterId = option.id">
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" :disabled="!selectedCharacterId" @click="handleApplyCharacterInfo">
            填入角色介绍
          </el-button>
        </div>
      </template>
...
const emit = defineEmits<{
  (
    e: "update-field",
    payload: { field: BasicInfoField; value: string | string[] },
  ): void;
}>();
...
const bindTagsPasteHandler = async () => {
  ...
  tagsPasteCleanup = bindDelimitedPaste(tagsSelectRef.value?.$el, (values) => {
    updateField(
      "tags",
      mergeUniqueValues(props.character.data.tags || [], values),
    );
  });
};
...
const loadCharacterInfoOptions = () => {
  const collection = readLocalStorageJSON<CharacterCollection>(
    LOCAL_STORAGE_KEY_CHARACTER_MANAGER,
  );
  ...
  characterInfoOptions.value = Object.values(collection.characters)
    .sort((a, b) => {
      ...
      return (a.data.chineseName || "").localeCompare(
        b.data.chineseName || "",
        "zh-Hans-CN",
      );
    })
    .map((item) => ({
      ...
      summary:
        [
          splitLines(item.data.identity).join("；"),
          splitLines(item.data.background)[0] || "",
          String(item.data.mbti ?? "").trim(),
        ]
          .filter(Boolean)
          .join(" ｜ ") || "暂无摘要",
      content: JSON.stringify(
        serializeCharacterInfo(item, { includeIdentityAsArray: true }),
        null,
        2,
      ),
    }));
...
const handleApplyCharacterInfo = async () => {
  const selectedOption = characterInfoOptions.value.find(
