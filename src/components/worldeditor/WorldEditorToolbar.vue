      ...
      <template #header-actions>
        <div class="split-create-actions">
          <el-tooltip content="新增地标" placement="top" :show-arrow="false" :offset="8" :hide-after="0">
            <button @click="emit('add', 'landmark')"
...
const emit = defineEmits<{
  ...
  (
    e: "edit",
    item: Project | EnhancedLandmark | EnhancedForce | EnhancedRegion,
  ): void;
...
const buildLandmarkTree = (
  projectLandmarks: EnhancedLandmark[],
  ...
  projectLandmarks.forEach((landmark) => {
    const regionColor = landmark.regionId
      ? regionColorMap.get(landmark.regionId)
      : undefined;
...
const treeData = computed(() => {
  return props.projects.map((project) => {
    const projectLandmarks = props.landmarks.filter(
      (l) => l.projectId === project.id,
    );
    const projectRegions = props.regions.filter(
      (r) => r.projectId === project.id,
    );
    const projectForces = props.forces.filter(
      (f) => f.projectId === project.id,
    );
    const regionColorMap = new Map(
      projectRegions.map((region) => [region.id, region.color]),
    );
...
const handleNodeDrop = (
  draggingNode: any,
  dropNode: any,
  dropType: ActualNodeDropType,
) => {
  const success = props.dragDropHandlers.handleNodeDrop(
    draggingNode,
    dropNode,
    dropType,
  );
...
const handleAddCommand = (
  command: "project" | "landmark" | "region" | "force",
) => {
  emit("add", command);
};
</script>
